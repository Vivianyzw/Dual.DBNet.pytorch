...
class DualTrainer(BaseTrainer):
    def __init__(self, config, model, criterion, train_loader, validate_loader, metric_cls, post_process=None):
        super(DualTrainer, self).__init__(config, model, criterion)
    ...
    def _train_epoch(self, epoch):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            if self.config['lr_scheduler']['type'] == 'WarmupPolyLR':
                self.scheduler.step()
            # acc iou
            fore_score_shrink_map = cal_text_score(preds[:, 0, :, :], batch['fore_shrink_map'], batch['fore_shrink_mask'], running_metric_text,
                                              thred=self.config['post_processing']['args']['thresh'])
            back_score_shrink_map = cal_text_score(preds[:, 3, :, :], batch['back_shrink_map'], batch['back_shrink_mask'],
                                              running_metric_text,
                                              thred=self.config['post_processing']['args']['thresh'])
            ...
            acc = (fore_score_shrink_map['Mean Acc'] + back_score_shrink_map['Mean Acc'])/2
            iou_shrink_map =(fore_score_shrink_map['Mean IoU'] + back_score_shrink_map['Mean IoU'])/2
            ...
            if self.tensorboard_enable and self.config['local_rank'] == 0:
                ...
                if self.global_step % self.show_images_iter == 0:
                    ...
                    fore_shrink_labels = batch['fore_shrink_map']
                    fore_threshold_labels = batch['fore_threshold_map']
                    fore_shrink_labels[fore_shrink_labels <= 0.5] = 0
                    fore_shrink_labels[fore_shrink_labels > 0.5] = 1
                    fore_show_label = torch.cat([fore_shrink_labels, fore_threshold_labels])
                    fore_show_label = vutils.make_grid(fore_show_label.unsqueeze(1), nrow=cur_batch_size, normalize=False, padding=20, pad_value=1)
                    self.writer.add_image('TRAIN/fore_gt', fore_show_label, self.global_step)

                    back_shrink_labels = batch['back_shrink_map']
                    back_threshold_labels = batch['back_threshold_map']
                    back_shrink_labels[back_shrink_labels <= 0.5] = 0
                    back_shrink_labels[back_shrink_labels > 0.5] = 1
                    back_show_label = torch.cat([back_shrink_labels, back_threshold_labels])
                    back_show_label = vutils.make_grid(back_show_label.unsqueeze(1), nrow=cur_batch_size,
                                                       normalize=False, padding=20, pad_value=1)
                    self.writer.add_image('TRAIN/back_gt', back_show_label, self.global_step)
