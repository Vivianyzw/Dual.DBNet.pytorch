...
class DualBaseDataSet(Dataset):
    ...
    def __init__(self, data_path: str, img_mode, pre_processes, filter_keys, ignore_tags, transform=None,
        ...
        item_keys = ['img_path', 'img_name', 'fore_text_polys', 'fore_texts', 'fore_ignore_tags', 'back_text_polys', 'back_texts', 'back_ignore_tags']
        for item in item_keys:
            assert item in self.data_list[0], 'data_list from load_data must contains {}'.format(item_keys)
    ...
    def __getitem__(self, index):
        try:
            ...
            if self.transform:
                data['img'] = self.transform(data['img'])
            data['fore_text_polys'] = data['fore_text_polys'].tolist()
            data['back_text_polys'] = data['back_text_polys'].tolist()
            if len(self.filter_keys):
                data_dict = {}
                for k, v in data.items():
                    if k not in self.filter_keys:
