...
class DualDetDataset(DualBaseDataSet):
    """
    数据标注分为前景文字标注和背景文字标注
    """
    def __init__(self, data_path: str, img_mode, pre_processes, filter_keys, ignore_tags, transform=None, **kwargs):
    ...
    def load_data(self, data_path: str) -> list:
        ...
        for path in data_path:
            ...
            for gt in tqdm(content['data_list'], desc='read file {}'.format(path)):
                ...
                fore_polygons = []
                fore_texts = []
                fore_illegibility_list = []
                fore_language_list = []
                back_polygons = []
                back_texts = []
                back_illegibility_list = []
                back_language_list = []
                for fore_annotation in gt['fore_annotations']:
                    if len(fore_annotation['polygon']) == 0 or len(fore_annotation['text']) == 0:
                        continue
                    if len(fore_annotation['text']) > 1 and self.expand_one_char:
                        fore_annotation['polygon'] = expand_polygon(fore_annotation['polygon'])
                    fore_polygons.append(fore_annotation['polygon'])
                    fore_texts.append(fore_annotation['text'])
                    fore_illegibility_list.append(fore_annotation['illegibility'])
                    fore_language_list.append(fore_annotation['language'])
                    if self.load_char_annotation:
                        for char_annotation in fore_annotation['chars']:
                            if len(char_annotation['polygon']) == 0 or len(char_annotation['char']) == 0:
                                continue
                            fore_polygons.append(char_annotation['polygon'])
                            fore_texts.append(char_annotation['char'])
                            fore_illegibility_list.append(char_annotation['illegibility'])
                            fore_language_list.append(char_annotation['language'])

                for back_annotation in gt['back_annotations']:
                    if len(back_annotation['polygon']) == 0 or len(back_annotation['text']) == 0:
                        continue
                    if len(back_annotation['text']) > 1 and self.expand_one_char:
                        back_annotation['polygon'] = expand_polygon(back_annotation['polygon'])
                    back_polygons.append(back_annotation['polygon'])
                    back_texts.append(back_annotation['text'])
                    back_illegibility_list.append(back_annotation['illegibility'])
                    back_language_list.append(back_annotation['language'])
                    if self.load_char_annotation:
                        for char_annotation in back_annotation['chars']:
                            if len(char_annotation['polygon']) == 0 or len(char_annotation['char']) == 0:
                                continue
                            back_polygons.append(char_annotation['polygon'])
                            back_texts.append(char_annotation['char'])
                            back_illegibility_list.append(char_annotation['illegibility'])
                            back_language_list.append(char_annotation['language'])


                data_list.append({'img_path': img_path, 'img_name': gt['img_name'],
                                  'fore_text_polys': np.array(fore_polygons), 'fore_texts': fore_texts, 'fore_ignore_tags': fore_illegibility_list,
                                  'back_text_polys': np.array(back_polygons), 'back_texts': back_texts, 'back_ignore_tags': back_illegibility_list})
        return data_list
